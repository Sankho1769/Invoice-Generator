...
def generate_invoice():
    ...
    messagebox.showinfo("Invoice Complete", "Invoice has been generated")
...
def export_to_excel():
    if not invoice_list:
        messagebox.showwarning("No Data", "Please add items before exporting.")
        return
    ...
    sheet.append(["", "", "Sales Tax (10%)", tax_amount])
    sheet.append(["", "", "Total", total])

    file_name = "Invoice_" + datetime.datetime.now().strftime("%Y-%m-%d-%H%M%S") + ".xlsx"
    wb.save(file_name)
...
export_excel_button = tkinter.Button(frame, text="Export to Excel", command=export_to_excel)
